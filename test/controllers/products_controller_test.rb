...
class ProductsControllerTest < ActionDispatch::IntegrationTest
  ...
  #Authenticated endpoints testing like this would obviously be relevant for all authenticated controllers,
  # and should be moved to a single test class.
  def setup
    #Setting up a user
  ...
  test "unauthenticated users should not have access to the controller" do
    get 'http://localhost:3111/api/v1/products', headers:{ Authorization: 'Bearer ij3o9028j89j2jfaojfd89920jfoijsxbgb09'}
    assert_response 401
  end

  test "users with valid jwt_tokens should have access to the controller" do
    get 'http://localhost:3111/api/v1/products', headers:{ Authorization: @jwt_token}
    assert_response 200
  end
